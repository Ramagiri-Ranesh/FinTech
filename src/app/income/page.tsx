...
import { Plus, Trash2, Edit2, Wallet, Check, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function IncomePage() {
  ...
  const fetchIncomes = async () => {
    setLoading(true);
    try {
      const data = await apiGet(`/api/income?month=${viewMonth}&year=${viewYear}`);
  ...
  const computeMonthlyTotal = () => {
    return incomes.reduce((acc, curr) => {
      ...
      if (curr.frequency === "One-time") return acc + curr.amount;
      return acc + val;
    }, 0);
  };
  ...
  return (
    <div className="font-[Inter] text-[#e2e2e8]">
      <header className="mb-10">
        ...
        <p className="text-sm text-[#bac9cc]">Manage your salary and side revenues — resets every month</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel p-5 md:p-8 rounded-2xl flex flex-col justify-center relative overflow-hidden h-48 md:h-[240px]">
          ...
          <p className="text-xs text-[#849396] mt-2 z-10">{MONTH_NAMES[viewMonth]} {viewYear}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 glass-panel p-5 md:p-8 rounded-2xl relative min-h-[240px]">
          <h3 className="text-lg font-medium mb-6">Add Income for {MONTH_NAMES[viewMonth]} {viewYear}</h3>
      ...
      {/* List */}
      <h3 className="text-lg font-medium mt-12 mb-6 text-white border-b border-[#3b494c]/50 pb-4">
        Income Sources — {MONTH_NAMES[viewMonth]} {viewYear}
      </h3>

      {loading ? (
        <div className="text-[#00daf3] p-10 text-center">Loading...</div>
      ) : (
        <div className="flex flex-col gap-4">
          ...
          {incomes.length === 0 && (
            <div className="text-sm text-[#849396] text-center mt-10 border border-dashed border-[#3b494c] rounded-xl p-10">
              No income sources for {MONTH_NAMES[viewMonth]} {viewYear}. Add one above.
            </div>
          )}
        </div>
      )}
